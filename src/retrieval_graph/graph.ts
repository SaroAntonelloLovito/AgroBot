...
function routeQuery(state: typeof AgentStateAnnotation.State): "createResearchPlan" | "askForMoreInfo" | "respondToGeneralQuery" {
...
async function respondToGeneralQuery(
  ...
  const configuration = ensureAgentConfiguration(config);
  const model = await loadChatModel(configuration.queryModel);
  const systemPrompt = configuration.generalSystemPrompt.replace("{logic}", state.router.logic);
  const messages = [
    { role: "system", content: systemPrompt },
    ...state.messages
  ];
...
async function respond(
  ...
  const prompt = configuration.responseSystemPrompt.replace("{context}", context);
  const messages = [
...
const builder = new StateGraph({
  ...
  .addConditionalEdges("analyzeAndRouteQuery", routeQuery, ["askForMoreInfo", "respondToGeneralQuery", "createResearchPlan"])
